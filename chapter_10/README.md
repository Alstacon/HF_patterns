...
— Управляет изменением поведения объекта контекста (класса с несколькими внутренними состояниями) при изменении его внутреннего состояния.

___
🎶
- В отличие от [Стратегии](chapter_1) клиент не имеет доступа к изменению состояния контекста, тогда как в стратегии
клиент определяет объект стратегии, который будет связан с контекстом
- Несколько объектов контекста могут одновременно использовать объекты состояния
- Объекты состояния не могут обладать собственным внутренним состоянием
- Каждое состояние присваивается статической переменной, а если состояние должно использовать методы контекста,
ссылка на контекст передается в методе `handler()`
